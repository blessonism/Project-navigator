...
在 Supabase Edge Functions 中部署 `import-project-metadata`，并设置以下函数环境变量：

- `IMPORT_SESSION_PASSCODE`（必填）：导入口令
- `OPENAI_API_KEY`（可选）：AI 结构化生成
- `OPENAI_BASE_URL`（可选）：默认 `https://api.openai.com/v1`
- `OPENAI_MODEL`（可选）：默认 `gpt-4.1-mini`

也可在管理后台「显示与导入设置」中开启“前端 AI 配置覆盖”，临时传入 `apiKey/baseUrl/model`，用于会话级调试与切换，并支持拉取模型与快速测试。
